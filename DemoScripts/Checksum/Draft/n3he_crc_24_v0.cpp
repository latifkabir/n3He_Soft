...
int Crc32_ComputeFile(FILE *file, unsigned *outCrc32) {
...
{
	FILE *fp = fopen("crc_tempo", "w");
	if (fp == 0){
		perror("crc_tempo"); exit(1);
	}
	...
	fclose(fp);
	FILE *fp2 = fopen("crc_tempo", "r");
	Crc32_ComputeFile(fp2,&my_value);
	

	fprintf(stdout, "%12d %08x %08x %s crc32:%08x\n", 
		es->data[4], es->data[5], es->data[7], 
			my_value==es->data[5]&& my_value==es->data[7]? "GOOD": "BAD", my_value);
