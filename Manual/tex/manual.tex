...
On basestar the data is being transferred and saved to the directories: \\ \textcolor{magenta}{ /mnt/idata01/data/ } (run number: 1-23661),\\ \textcolor{magenta}{ /mnt/idata02/data/ } (run number: 23662 -31839 ),\\ \textcolor{magenta}{ /mnt/idata03/data/ } (run number: 31840- )\\and the analysis library is compiled in a shared directory \textcolor{magenta}{ /home/npdg/n3He/libn3He/lib} 
...
In the analysis we recommend to skip the last event to be safe in case the last event of the run is a partial event (though every effort has been done to avoid this).
...
Out of 48 clean ADC channels, only 36 ADC channels are connected to the pre-amps (ADC channels 0 to 17 and channels 24 to 41). Moreover there are five bad channels: DAQ21- channels 5 \& 6(they are combined on channel 6), DAQ22 channels 35 and DAQ 24 channel-35,39(these have opposite polarity). 
